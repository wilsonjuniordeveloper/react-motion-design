...
function ItemComponent({ item, id }) {
  ...
  return (
    <MotionScene name={`photo-${id}`} onClick={withTransition(callback)}>
      <Container>
        <SharedElement.Image
          style={{width: '95%', padding: 20, borderRadius: "20px"}}
          alt=""
          src={item.photo}
          animationKey="image"
        />
